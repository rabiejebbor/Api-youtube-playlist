...
$(document).ready(function () {
  ...
  function getPlaylistId() {
    playlistId =
      link.value.match(new RegExp("list=" + "(.*)" + "&")) ||
      link.value.slice(link.value.indexOf("list=") + 5, link.value.length);
    // let playlistId = link.value.match(new RegExp("list=" + "(.*)" + "&"))[1];
    if (Array.isArray(playlistId)) playlistId = playlistId[1];
  ...
  function loadVideos(URL, options) {
    $.getJSON(URL, options, function (params) {
      // videosArray = [["Title", "Channel", "Video URL", "Publish Date"]];
  ...
  async function start() {
    ...
    if (!playlistId) {
      console.log("error");
      spinner.innerHTML = `<i class="fas fa-times" style="color:red"></i>`;
      return;
    }
    getPlaylistTitle();
    await sleep(500);
    ...
    console.log("ready");
    spinner.innerHTML = `<i class="fas fa-check" style="color:green"></i>`;
...
function makeApiCall() {
  gapi.client.sheets.spreadsheets
    ...
    .then((response) => {
      ...
      sheetsTitle = response.result.sheets[0].properties.title;
...
function makeSecondApiCall() {
  var params = {
    ...
    range: sheetsTitle, // TODO: Update placeholder value.
  ...
  request.then(
    function (response) {
      ...
      console.log(response.result);
      console.log("saved");
      saved.innerHTML = `<i class="fas fa-check" style="color:green"></i>`;
      signIn.setAttribute("disabled", "true");
    },
    function (reason) {
      console.error("error: " + reason.result.error.message);
      saved.innerHTML = `<i class="fas fa-times" style="color:red"></i>`;
    }
  );
  handleSignOutClick();
}
...
function handleSignInClick(event) {
  ...
  saved.innerHTML = `<div class="spinner-border text-primary" role="status">
  <span class="sr-only">Loading...</span>
</div>`;
}
...
// bugs with playlist id extraction (link=...& or link=...)  when putting  <i class="fas fa-times"></i> in the link it accepts it as a lit id
